...
# Run EXPLAIN to check query plan
explain_plan_result2 <- dbGetQuery(con,
  "EXPLAIN SELECT COUNT(id) FROM highlights where source_type='123'"
)

# Convert explain plan (which is a data frame with 1 column) to a character vector
explain_plan_text2 <- paste(explain_plan_result2[[1]], collapse = "\n")
...
explain_str2 <- c("=== EXPLAIN Plan 2 ===", explain_plan_text2, "")
...
writeLines(c(explain_str, explain_str2), con = csv_file_path)
