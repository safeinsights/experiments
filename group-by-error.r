...
example_row_query_result <- dbGetQuery(con,
  "SELECT * FROM highlights GROUP BY id LIMIT 1"
)
print("Finished row query.")

# How many unique highlights exist
# Using GROUP BY causes this to error - probably out-of-memory
highlights_count_query_result <- dbGetQuery(con,
  "SELECT COUNT(*) as highlights_count FROM highlights GROUP BY id"
)
print("Finished highlights count query.")
...
print("About to print count query result...")
print(highlights_count_query_result)
highlights_count_str <- capture.output({
  cat("=== Highlights Count Query Result ===\n")
  print(highlights_count_query_result)
  cat("\n")
})
print("Finished count1 output.")
writeLines(c("Example Row Query Result:", example_row_str,
             "Unique Users Query Result:", highlights_count_str),
           con = csv_file_path,
           sep = "\n")
